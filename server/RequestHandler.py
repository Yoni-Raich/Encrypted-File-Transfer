...
class RequestHandler:
    def __init__(self):
        ...
        self.request_structur = None
        self.file_structure = []

    def new_request(self, receive_request: RequestStructure):
        self.request_structur = receive_request

    def create_response(self, client_socket: socket.socket):
        code = self.request_structur.code
        client_id = self.request_structur.client_id
        payload = self.request_structur.payload
    ...
    def handle_public_key(self, client_id, payload):
        name = self.request_structur.get_name()
        public_key = self.request_structur.get_public_key()
    ...
    def handle_file_transfer(self, client_id, client_socket):
        packet_size = len(self.request_structur.payload)
        self.file_structure = self.request_structur.get_file_structure()
        num_of_packets = self.file_structure['total_packet']
        for i in range(1, num_of_packets):
            data = client_socket.recv(packet_size)
            new_file_structure = self.request_structur.get_file_structure(data)
